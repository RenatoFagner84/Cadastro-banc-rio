...
const atualizarTransacao = async(req,res) =>{
  ...
  try {  
    const transacao = await query('select * transacoes where usuario_id = $1 AND id = $2',[usuario.id,id]);
    
     if(transacao.rowCount <= 0){
      return res.status(400).json({mensagem:'Transação não existe.'});
     }

     const categoria = await query('select * categorias where id = $1',[categoria_id]);
     ...
     if(categoria.rowCount <= 0){
      ...
      const transacaoAtualizada = await query(queryAtualizcaoTransacao, parametrosAtualizacao);
